...
class Review(models.Model):
    ...
    spot = models.ForeignKey(
        Spot, on_delete=models.CASCADE, related_name="reviews", verbose_name="スポット"
    )
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="レビュー者")
    rating = models.IntegerField(
        validators=[MinValueValidator(1), MaxValueValidator(5)], verbose_name="評価"
    )
...
class SpotView(models.Model):
    ...
    spot = models.ForeignKey(
        Spot, on_delete=models.CASCADE, related_name="spot_views", verbose_name="スポット"
    )
